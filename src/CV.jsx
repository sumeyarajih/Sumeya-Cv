...
export default function CV() {
  ...
  return (
    <div className="min-h-screen bg-pink-50 flex justify-center py-8 px-4 font-body">
      ...
      >
        ...
        <aside
          ...
          style={{ background: "linear-gradient(160deg, #831843 0%, #9d174d 60%, #be185d 100%)" }}
        >
          {/* ── Photo + Name + Title ── */}
          <div className="flex flex-col items-center pt-7 pb-5 px-5 text-center border-b border-pink-400/30">
            ...
            <div
              ...
              className="no-print w-[90px] h-[90px] rounded-full border-4 border-pink-300/70 overflow-hidden flex items-center justify-center mb-3 shadow-lg cursor-pointer hover:opacity-80 transition-opacity bg-pink-200/30"
            >
              {photoUrl ? (
                <img src={photoUrl} alt="Profile" className="w-full h-full object-cover" />
              ) : (
                <User size={40} className="text-pink-200/80" />
              )}
            </div>

            {/* Photo shown in print (non-interactive) */}
            {photoUrl && (
              <div className="print-only w-[90px] h-[90px] rounded-full border-4 border-pink-300/70 overflow-hidden mb-3 shadow-lg mx-auto">
                <img src={photoUrl} alt="Profile" className="w-full h-full object-cover" />
              </div>
            )}
            ...
            <div>
              ...
              <ul className="space-y-1.5">
                {certifications.map((c, i) => (
                  <li key={i} className="flex gap-1.5 items-start text-[10px] text-pink-100 leading-snug">
